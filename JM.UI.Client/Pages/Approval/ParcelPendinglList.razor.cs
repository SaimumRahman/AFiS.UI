...
{
    public partial class ParcelPendinglListComponent : PosComponentBase
    {
        ...
        private async Task LoadPendingApproval()
        {
            try
            {
                ...
                PendingApproval = await _serviceUnitOfWork.PendingApprovalService.GetAll(Convert.ToInt32(result.Value));
            }
            catch (Exception ex)
            {
        ...
        {
            var confirm = await dialogService.Confirm(
                $"Approve '{item.WorkflowName}' at level '{item.CurrentLevelName}'?",
                "Confirm Approval",
                new ConfirmOptions
                {
        ...
        protected void EditParcel(PendingApprovalDTO parcel)
        {
            NavigationManager.NavigateTo($"/ParcelAdd/{parcel.EntityId}");
        }
        public void Dispose()
        {
