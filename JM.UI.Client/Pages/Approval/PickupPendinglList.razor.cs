...
{
    public partial class PickupPendinglListComponent : PosComponentBase
    {
        ...
        private async Task LoadPendingApproval()
        {
            try
            {
                ...
                PendingApproval = await _serviceUnitOfWork.PendingApprovalService.GetAllPendingPickup(Convert.ToInt32(result.Value));
            }
            catch (Exception ex)
            {
